#!/usr/bin/env bash
# schedule-eod.sh — Manages daily 6:00 PM EOD capture
# Usage: bash .tools/schedule-eod.sh install|remove|status|run-now
set -e
SCRIPT_DIR="$(dirname "${BASH_SOURCE[0]}")"
VAULT="$(cd "$SCRIPT_DIR/.." && pwd)"
TOOLS="$VAULT/.tools"
PLIST_ID="com.llm-wiki-vault.eod"
PLIST_PATH="$HOME/Library/LaunchAgents/$PLIST_ID.plist"
LOG="$TOOLS/eod.log"
RUNNER="$TOOLS/run-eod.sh"
OS=$(uname -s)
case "${1:-}" in
install)
    if [ "$OS" = "Darwin" ]; then
...
run-now) bash "$RUNNER" ;;
*) echo "Usage: bash .tools/schedule-eod.sh install|remove|status|run-now"; exit 1 ;;
esac
