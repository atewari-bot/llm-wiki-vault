...
def main():
    ...
    missing   = find_missing_person_pages(pages, vault / "wiki")
    sequences = find_implicit_sequences(pages, max_results=2)
    anomaly   = find_anomaly(pages)

    print(f"   Bridges: {len(bridges)}  Missing pages: {len(missing)}  Sequences: {len(sequences)}")
    ...
    # ── Write report ──────────────────────────────────────────
    report = render_discoveries(today_str, pages, bridges, missing, sequences, anomaly)
...
def find_bridge_connections(pages: list[dict], max_results: int = 3) -> list[dict]:
    ...
    for i, pa in enumerate(pages):
        for pb in pages[i+1:]:
            ...
            skip_tags   = {"knowledge-graph", "concept", "person", "tool", "moc"}
...
def _guess_relationship(pa: dict, pb: dict) -> str:
    ...
    if any(w in a_body for w in ["part of", "component", "subset", "type of"]):
        return "part_of"
    if any(w in b_body for w in ["created by", "invented by", "developed by"]) \
       and pb["type"] == "person":
        return "created_by"
    return "relates_to"
...
def find_missing_person_pages(pages: list[dict], wiki_dir: Path) -> list[dict]:
    """
    Find proper nouns mentioned 3+ times across concept pages
    that don't have a person page.
    """
    existing_people = {
        f.stem.lower()
        for f in (wiki_dir / "people").glob("*.md")
        if not f.name.startswith(".")
    }

    # Count proper noun mentions across all concept pages
    name_mentions = defaultdict(list)
    for page in pages:
        if page["type"] == "person":
            continue
        # Find capitalized word pairs (likely names)
        names = re.findall(r"\b([A-Z][a-z]+ [A-Z][a-z]+)\b", page["body"])
        for name in names:
            if name.lower() not in existing_people:
                name_mentions[name].append(page["id"])

    missing = []
    for name, sources in name_mentions.items():
        if len(sources) >= 2:
            missing.append({
                "name":    name,
...
def render_discoveries(today_str, pages, bridges, missing, sequences, anomaly) -> str:
    ...
    if missing:
        lines += ["## Missing pages", ""]
        for m in missing:
            sources = ", ".join(f"[[{s}]]" for s in m["sources"])
            lines += [
                f"### [[{m['name']}]] has no wiki page",
                f"Referenced {m['count']} times in: {sources}",
                f"**Action:** `ingest` an article about them, or create a stub.",
                "",
            ]
    ...
    if not any([bridges, missing, sequences, anomaly]):
        lines += [
